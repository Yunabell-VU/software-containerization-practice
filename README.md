...
Respectively, **webapp.yaml** configured a load balancer named outlets-service which will select pods with frontend label to associate with. It listens on port 8080 of the cluster IP and redirects the traffic to the port 8080 of the associated pods.

**api.yaml** configured a load balancer named api-service which will select pods with the api label to associate with. It listens on port 5000 of the cluster IP and redirects the traffic to the port 5000 of the associated pods.
