...
### Load Balancer
Two sevices of type load balancer are configured.

Respectively, **webapp.yaml** configured a load balancer named outlets-service which will select pods with label **app=outlets** to associate with. It listens on port 8080 of the cluster IP and redirects the traffic to the port 8080 of the associated pods.

**api.yaml** configured a load balancer named api-service which will select pods with label **app=api** to associate with. It listens on port 5000 of the cluster IP and redirects the traffic to the port 5000 of the associated pods.
